...
const transformNextConfig = (zipPath) => {
  ...
  const [entry] = entries.filter((e) => e.name === 'nest.config.js')
  if (entry) {
    entry.setData(
      Buffer.from(
        entry
    ...
    console.log('TransformNextConfig: nest.config.js not exist.')
  }
  zip.writeZip()
}
